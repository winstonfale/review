  ...
  <div class="row justify-content-center">
    <div class="col-md-9">
      <div class="card">
        ...
        <table class="table">
          <thead>
            <th width="40%" @click="reset(0)" class="cursor">
              Campaign Name (Reset)
            </th>
            <th>Clicks</th>
            <th>Conversions</th>
          ...
          <tr
            ...
            v-for="(campaign, index) in containers"
            :key="campaign.name + index + '1'"
          >
            <td
              width="40%"
              class="cursor"
              @click="insertToContainer(campaign, false)"
            >
              {{ getBreadCrumbs(campaign.tag) }}
              {{ campaign.name | filterName }}
            </td>
          ...
          >
            <td width="40%" class="cursor" @click="insertToContainer(campaign)">
              {{ getBreadCrumbs(campaign.tag) }}
              {{ campaign.name | filterName }}
            </td>
            <td>{{ campaign.clicks }}</td>
  ...
  methods: {
    fetch(groupBy = "site_id", siteId = null) {
      axios
        ...
        .then((res) => [(this.campaigns = res.data)])
        .catch((err) => {});
    },
      ...
      try {
        window.history.replaceState(
          ...
          window.location.pathname +
            "?" +
            new URLSearchParams(this.filters).toString()
        );
      } catch (e) {}
      axios
        .post("/clicks", {
    ...
    insertToContainer(campaign, pushToContainer = true) {
      ...
      if (campaign.tag === "s2") {
        ...
        this.containers = this.containers.filter(
          (item) =>
            !(item.tag === "s3" || item.tag === "s4" || item.tag === "s5")
        );
      }
